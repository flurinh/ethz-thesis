...
These representations provide the infrastructure for the research contributions that follow. \autoref{ch:mogrn} introduces MOGRN, a standardized numbering system for microbial rhodopsins, validated against 129 structures assembled and aligned through ProtOS. \autoref{ch:lambda} presents LAMBDA, a cross-family spectral prediction model whose training data---pocket graphs, per-residue embeddings, GRN annotations, and measured $\lambda_\text{max}$ values---was built entirely through the processors described here. The Opsin Atlas extends those predictions to 47,700 sequences, a scale made practical because ProtOS manages the pipeline from sequence retrieval through spectral prediction.
