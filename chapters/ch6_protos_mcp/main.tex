...
The following sections introduce the architecture briefly, then demonstrate it on the same rhodozyme from \autoref{ch:rhodozyme}.
...
To demonstrate ProtOS-MCP, the same rhodozyme from \autoref{ch:rhodozyme} is used as a starting point. The design preserves the retinal-binding domain, which means spectral tuning remains possible: mutations in the chromophore environment can shift the absorption wavelength without disrupting the designed catalytic site. The question posed through conversation: can the rhodozyme's predicted absorption be shifted toward longer wavelengths by transplanting binding pocket residues from the human long-wave sensitive opsin?
